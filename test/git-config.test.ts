...
test("Git v3 config is credential-free and uses the complete reviewed path", async () => {
	await withTempHome(async (agentDir) => {
		...
		writeFileSync(localConfigPath(), JSON.stringify(gitSettings()), {
			mode: 0o600,
		});
		...
		assert.equal(
			config.backend.profile.remote,
			"git@github.com:user/pi-sync.git",
		);
		...
		assert.doesNotMatch(
			JSON.stringify(config.backend),
			/password|secretAccessKey/u,
		);
	});
});
...
test.each([".", "./", " ./ "])(
	...
	async (rootPath) => {
		await withTempHome(async (agentDir) => {
			...
			writeFileSync(
				localConfigPath(),
				JSON.stringify(gitSettings(rootPath, "main")),
				{
					mode: 0o600,
				},
			);
			...
			writeFileSync(
				localConfigPath(),
				JSON.stringify(gitSettings("./", "main")),
				{ mode: 0o600 },
			);
			assert.equal(
				statePathForConfig(await loadConfig()),
...
test("Git state identity changes by remote, branch, and path but not setup name", async () => {
	await withTempHome(async (agentDir) => {
		...
		writeFileSync(localConfigPath(), JSON.stringify(firstSettings), {
			mode: 0o600,
		});
		...
		(
			renamed.syncSetups as Record<string, typeof renamed.syncSetups.home>
		).renamed = renamed.syncSetups.home;
		...
		assert.equal(
			statePathForConfig(await loadConfig()),
			statePathForConfig(first),
		);

		writeFileSync(
			localConfigPath(),
			JSON.stringify(gitSettings("other/home")),
			{ mode: 0o600 },
		);
		assert.notEqual(
			statePathForConfig(await loadConfig()),
