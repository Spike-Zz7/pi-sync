...
const packageRoot = existsSync(resolve("packages/pi-sync"))
	? resolve("packages/pi-sync")
	: resolve(".");
const builderUrl = pathToFileURL(
	join(packageRoot, "scripts/build-runtime.mjs"),
).href;
...
async function loadBuilder(): Promise<RuntimeBuilder> {
	return (await import(
		`${builderUrl}?test=${crypto.randomUUID()}`
	)) as RuntimeBuilder;
}
...
test("generated runtime preserves every first-use import boundary", async () => {
	...
	try {
		const metadata = await builder.buildRuntime({
...
test("generated runtime is mapped, external, self-contained, and loadable by Pi", async () => {
	...
	try {
		...
		for (const runtimePath of files.filter((path) => path.endsWith(".ts"))) {
			...
			assert.ok(
				files.includes(`${runtimePath}.map`),
				`missing map for ${runtimePath}`,
			);
		}
		assert.match(
		...
		const { ctx } = createMockContext({
			...
			input: async (title: string) => {
				titles.push(title);
				if (title.startsWith("Sync setup name")) return "default";
				if (title.includes("Git remote URL"))
					return "git@github.com:example/settings.git";
				return undefined;
			},
